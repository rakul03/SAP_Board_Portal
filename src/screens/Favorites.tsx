...
export function Favorites({ onOpenDetail, onBack }: FavoritesProps) {
  const {
    initiatives,
    owners,
    favorites,
  ...
  const favoriteInitiatives = useMemo(
    () => initiatives.filter((i) => favorites.includes(i.id)),
    [initiatives, favorites],
  );

  const filtered = useMemo(() => {
    let out = [...favoriteInitiatives];
  ...
  }, [favoriteInitiatives, categoryFilter, ownerFilter, logDateFilter, statusFilter, search]);
  ...
  const handleExport = (scope: 'all' | 'view') => {
    const set = scope === 'all' ? favoriteInitiatives : filtered;
    const fname = `favorites_${todayStamp()}.xlsx`;
  ...
  return (
    <div className={styles.wrap}>
      <PageHeader
        title="Favorites"
        subtitle="Your starred initiatives across all categories"
        breadcrumbs={['SAP Board Portfolio', 'Favorites']}
        actions={
          <>
            {onBack && (
                  ...
                  >
                    <button onClick={() => handleExport('all')}>
                      ...
                      <span className={styles.exportCount}>
                        {favoriteInitiatives.length} records
                      </span>
                    </button>
